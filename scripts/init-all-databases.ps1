# PowerShell script to initialize all databases with sample data
# Run this after starting the containers with: docker-compose up -d

Write-Host "=== Initializing All NoSQL Databases ===" -ForegroundColor Green
Write-Host ""
...
# Initialize Redis
Write-Host "1. Initializing Redis..." -ForegroundColor Cyan
...
Write-Host "2. MongoDB initialized automatically" -ForegroundColor Green
Write-Host ""

# Initialize Cassandra
Write-Host "3. Initializing Cassandra..." -ForegroundColor Cyan
Write-Host "Waiting for Cassandra to be ready..." -ForegroundColor Yellow
Start-Sleep -Seconds 30

docker exec cassandra-nosql cqlsh -f /docker-entrypoint-initdb.d/init.cql
if ($LASTEXITCODE -ne 0) {
    Write-Host "Cassandra not ready yet, trying direct CQL..." -ForegroundColor Yellow
...
Write-Host "✓ Cassandra initialized" -ForegroundColor Green
Write-Host ""

# Initialize Neo4j
Write-Host "4. Initializing Neo4j..." -ForegroundColor Cyan
Write-Host "Waiting for Neo4j to be ready..." -ForegroundColor Yellow
Start-Sleep -Seconds 15

docker exec neo4j-nosql cypher-shell -u neo4j -p neo4j123 "MATCH (n) RETURN count(n) LIMIT 1" 2>$null
if ($LASTEXITCODE -ne 0) {
    Write-Host "Loading Neo4j data..." -ForegroundColor Yellow
    Get-Content init/neo4j-init.cypher | docker exec -i neo4j-nosql cypher-shell -u neo4j -p neo4j123
}
...
# Initialize Fuseki
Write-Host "5. Initializing Fuseki..." -ForegroundColor Cyan
Write-Host "Waiting for Fuseki to be ready..." -ForegroundColor Yellow
Start-Sleep -Seconds 10

# Load RDF data into Fuseki
docker exec fuseki-nosql curl -X POST -H "Content-Type: application/rdf+xml" -T /fuseki/init-data.ttl "http://localhost:3030/llm_nosql_dataset/data?default"
if ($LASTEXITCODE -ne 0) {
    Write-Host "Fuseki dataset may need manual initialization via web UI" -ForegroundColor Yellow
}

Write-Host "✓ Fuseki initialized" -ForegroundColor Green
Write-Host ""
...
Write-Host "  - MongoDB: localhost:27017 (admin/admin123)"
Write-Host "  - Cassandra: localhost:9042"
Write-Host "  - Neo4j: http://localhost:7474 (neo4j/neo4j123)"
Write-Host "  - Fuseki: http://localhost:3030 (admin/admin123)"
